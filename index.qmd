...
$$\alpha_t \sim N\left(a_{t|t}, P_{t|t}\right)$$ $$\alpha_{t+1} \sim N\left(a_{t+1}, P_{t+1}\right)$$ La idea es calcular $a_{t|t}$, $P_{t|t}$, $a_{t+1}$ y $P_{t+1}$ partir de $a_t$ y $P_t$ donde
...
$\hat{\alpha}_t$ y $V_t$ se calculan de forma recursiva a partir del estado inicial $\alpha_1 \sim N\left(a_1, P_1\right)$ como sigue: $$\begin{array}{ll}\quad r_{t-1}=Z_t^\prime F_t^{-1}v_t + L_t^\prime r_t & \hspace{3cm}N_{t-1} = Z_t^\prime F_t^{-1}Z_t + L_t^\prime N_tL_t \\ \hspace{2cm}\hat{\alpha}_t = a_t + P_tr_{t-1} & \hspace{4.5cm}V_t = P_t- P_tN_{t-1}P_t \\\end{array}$$ $$t=n,\cdots,1.$$ El algoritmo se inicializa con $r_n=0$ y $N_n=0.$
